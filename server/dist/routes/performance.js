...
router.post("/save-answer", async (req, res) => {
    try {
        ...
        const existingPerformance = await db
            ...
            .where(and(eq(desempenhoTemas.usuarioId, usuario_id), eq(desempenhoTemas.tema, tema)))
            .limit(1);
...
router.get("/", async (req, res) => {
    try {
        ...
        const totalQuestoes = temas.reduce((sum, t) => sum + (t.totalQuestoes || 0), 0);
        const totalAcertos = temas.reduce((sum, t) => sum + (t.acertos || 0), 0);
